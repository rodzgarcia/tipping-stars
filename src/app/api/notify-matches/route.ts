...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: matches } = await supabase
      .from('matches')
      .select('id, home_team, away_team, kickoff_at, round, tournament_id')
      .in('id', matchIds)
      .order('kickoff_at')

    if (!matches?.length) return NextResponse.json({ error: 'No matches found' }, { status: 400 })
    ...
    // Get approved members with email stored in profiles
    ...
    // Get profiles including email field
    ...
    // If service role available, get emails from auth.users
    let emailMap: Record<string, string> = {}
    if (serviceKey) {
      const { data: authUsers } = await supabase.auth.admin.listUsers()
      authUsers?.users?.forEach((u: any) => { emailMap[u.id] = u.email })
    } else {
      // Fall back to email stored in profiles table
    ...
    const matchRows = (matches as any[]).map((m: any) => {
      const kickoff = new Date(m.kickoff_at)
      const lockTime = new Date(kickoff.getTime() - lockMins * 60 * 1000)
      const kickoffStr = kickoff.toLocaleString(isPt ? 'pt-BR' : 'en-AU', { weekday: 'short', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit', timeZone: 'Australia/Sydney' }) + ' AEST'
      const lockStr = lockTime.toLocaleString(isPt ? 'pt-BR' : 'en-AU', { hour: '2-digit', minute: '2-digit', timeZone: 'Australia/Sydney' }) + ' AEST'
    ...
    for (const member of (members as any[])) {
      ...
      const subject = isPt
        ? `⚽ ${(matches as any[]).length} novo${(matches as any[]).length > 1 ? 's jogos' : ' jogo'} disponível — ${tourName}`
        : `⚽ ${(matches as any[]).length} new match${(matches as any[]).length > 1 ? 'es' : ''} to tip — ${tourName}`
      ...
      const html = '<!DOCTYPE html><html><body style="background:#0a0f0d;color:#e8f5ee;font-family:Arial,sans-serif;margin:0;padding:0">'
        ...
        + '<div style="text-align:center;margin-bottom:24px"><div style="font-size:28px;font-weight:900;letter-spacing:4px">⭐ ' + appName.toUpperCase() + ' ⭐</div>'
        + '<div style="color:#fbbf24;font-size:14px;margin-top:4px">' + tourName + '</div></div>'
        ...
        + '<p style="color:rgba(255,255,255,0.6);margin:0 0 20px"><strong style="color:#4ade80">'
        + (matches as any[]).length + (isPt ? ' novo' + ((matches as any[]).length > 1 ? 's jogos foram adicionados' : ' jogo foi adicionado') : ' new match' + ((matches as any[]).length > 1 ? 'es have been added' : ' has been added'))
        + '</strong> ' + (isPt ? 'em' : 'to') + ' <strong>' + tourName + '</strong>. ' + (isPt ? 'Não perca seus pontos!' : "Don't miss out on points!") + '</p>'
        + '<table style="width:100%;border-collapse:collapse;margin-bottom:20px">'
        + '<thead><tr>'
        + '<th style="padding:8px 14px;text-align:left;color:rgba(255,255,255,0.4);font-size:11px;background:#0a1a0a">' + (isPt ? 'JOGO' : 'MATCH') + '</th>'
        + '<th style="padding:8px 14px;text-align:left;color:rgba(255,255,255,0.4);font-size:11px;background:#0a1a0a">' + (isPt ? 'INÍCIO' : 'KICKOFF') + '</th>'
        + '<th style="padding:8px 14px;text-align:left;color:rgba(255,255,255,0.4);font-size:11px;background:#0a1a0a">' + (isPt ? 'BLOQUEIO' : 'LOCK') + '</th>'
        + '<th style="padding:8px 14px;text-align:left;color:rgba(255,255,255,0.4);font-size:11px;background:#0a1a0a">' + (isPt ? 'FASE' : 'ROUND') + '</th>'
        ...
        + (isPt ? '⚽ Apostar agora →' : '⚽ Tip Now →') + '</a></div>'
        + '<p style="text-align:center;color:rgba(255,255,255,0.2);font-size:12px;margin-top:16px">'
        + (isPt ? 'Você está participando de ' : "You're in ") + tourName + '.</p></div></body></html>'
      ...
      if (res.ok) sent++
      else { failed++; console.error('Resend error for', email, await res.text()) }
    }

    return NextResponse.json({ sent, failed, noEmail, total: (members as any[]).length })
  } catch (e: any) {
    ...
    for (const member of members) {
      ...
      const subject = isPt
        ? `⚽ ${matches.length} novo${matches.length > 1 ? 's' : ''} jogo${matches.length > 1 ? 's' : ''} disponíve${matches.length > 1 ? 'is' : 'l'} — ${tourName}`
        : `⚽ ${matches.length} new match${matches.length > 1 ? 'es' : ''} to tip — ${tourName}`
      ...
      const html = '<!DOCTYPE html><html><body style="background:#0a0f0d;color:#e8f5ee;font-family:Arial,sans-serif;margin:0;padding:0">'
        ...
        + (isPt
          ? '<strong style="color:#4ade80">' + matches.length + ' novo' + (matches.length > 1 ? 's jogo' : ' jogo') + (matches.length > 1 ? 's foram adicionados' : ' foi adicionado') + '</strong> em <strong>' + tourName + '</strong>. Não perca seus pontos!'
          : '<strong style="color:#4ade80">' + matches.length + ' new match' + (matches.length > 1 ? 'es have' : ' has') + ' been added</strong> to <strong>' + tourName + '</strong>. Don\'t miss out on points!')
        + '</p>'
        + '<table style="width:100%;border-collapse:collapse;margin-bottom:20px">'
        + '<thead><tr>'
      ...
      const res = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: 'Tipping Stars <notifications@tipping-stars.vercel.app>',
          to: email,
          subject,
          html,
        }),
